...
def load_lexicon(path):
    ...
    with open("./resources/unstressables.txt") as f:
        function_words = f.readlines()
    with open("./resources/additional_transcriptions.csv") as f:
        additional_transcriptions = {
            k: v for k, v in [line.split(",") for line in f.readlines()]
        }

    missing = pd.DataFrame.from_dict(
        to_nst_format(additional_transcriptions), orient="index", columns=COLUMNS,
    )
    df = pd.read_csv(
        path,
        encoding="utf-8",
